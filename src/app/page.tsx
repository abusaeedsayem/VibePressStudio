    ...
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <section id="products" className="py-20 px-4 md:px-6 max-w-[1280px] mx-auto w-full">
        ...
        <div className="grid grid-cols-1 gap-12 max-w-4xl mx-auto">
          {products.map((p) => {
            const isBlueprnt = p.id === "blueprnt";
            ...
            return (
              ...
              >
                ...
                <div className={`p-8 text-white relative overflow-hidden ${
                  isBlueprnt 
                    ? "bg-gradient-to-r from-sky-600 via-slate-900 to-indigo-900" 
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 mb-4 relative z-10">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-semibold bg-white/20 text-white px-3 py-1 rounded-full backdrop-blur-xs">
                        {isBlueprnt ? "100% Offline • Local-First" : "24 Core Features"}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-3.5 mb-3 relative z-10">
                    {p.logoImage && (
                      <img 
                        src={p.logoImage} 
                        alt={p.name} 
                        className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-slate-950 p-1.5 border border-white/30 shadow-lg object-contain shrink-0" 
                      />
                    )}
                    ...
                    {isBlueprnt ? (
                      <>
                        <div className="bg-white/10 backdrop-blur-xs rounded-xl p-3 text-center border border-white/15">
                          <div className="text-lg md:text-xl font-extrabold text-white">BLAKE3</div>
                          <div className="text-[11px] text-white/80 font-medium">Checksum Verification</div>
                        </div>
                        <div className="bg-white/10 backdrop-blur-xs rounded-xl p-3 text-center border border-white/15">
                          <div className="text-lg md:text-xl font-extrabold text-white">Atomic</div>
                          <div className="text-[11px] text-white/80 font-medium">Shot-Group Preservation</div>
                        </div>
                        <div className="bg-white/10 backdrop-blur-xs rounded-xl p-3 text-center border border-white/15">
                          <div className="text-lg md:text-xl font-extrabold text-white">3-2-1</div>
                          <div className="text-[11px] text-white/80 font-medium">Dual-Drive Backups</div>
                        </div>
                        <div className="bg-white/10 backdrop-blur-xs rounded-xl p-3 text-center border border-white/15">
                          <div className="text-lg md:text-xl font-extrabold text-white">ProRes</div>
                          <div className="text-[11px] text-white/80 font-medium">ProRes &amp; DNxHR Proxies</div>
                        </div>
                      </>
                    ) : (
                      <>
                ...
                <div className="p-8 flex flex-col flex-1">
                      ...
                      <>
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-bold text-foreground uppercase tracking-wider flex items-center gap-2">
                            <Sparkles className="h-4 w-4 text-sky-500" /> BLAKE3 Checksum Ingestion Workstage
                          </span>
                          <span className="text-[11px] text-emerald-600 dark:text-emerald-400 font-semibold bg-emerald-500/10 px-2.5 py-0.5 rounded-full border border-emerald-500/20">
                            Zero Bit-Rot Guarantee
                          </span>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-mono text-xs">
                          <div className="bg-background p-3.5 rounded-lg border border-sky-500/30 space-y-1.5">
                            <div className="text-[11px] font-bold uppercase text-sky-500 flex items-center gap-1.5">
                              <span className="w-1.5 h-1.5 rounded-full bg-sky-500" /> Source Media Card Ingest
                            </div>
                            <div className="text-muted-foreground break-all text-[11px]">
                              /Volumes/EOS_DIGITAL/DCIM/100EOS5D (RAW + JPG + MOV + XMP)
                            </div>
                          </div>

                          <div className="bg-background p-3.5 rounded-lg border border-emerald-500/30 space-y-1.5">
                            <div className="text-[11px] font-bold uppercase text-emerald-500 flex items-center justify-between">
                              <span className="flex items-center gap-1.5">
                                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> BLAKE3 Checksum Verified
                              </span>
                            </div>
                            <div className="text-foreground font-bold break-all text-[11px]">
                              /Volumes/NVMe_EDIT/2026-09-17_Shoot_01 (3-2-1 Mirrored)
                            </div>
                          </div>
                        </div>
                      </>
                  ...
                  {/* Product-Specific Feature Spotlight Tabs */}
                  <div className="mb-8">
                    <div className="flex items-center gap-2 border-b border-border pb-3 mb-4 overflow-x-auto no-scrollbar">
                      <button
                      ...
                      >
                        {isBlueprnt ? "Core Features (Workstage)" : "Core Architecture (24 Features)"}
                      </button>
                      <button
                      ...
                      >
                        {isBlueprnt ? "Multi-Platform Support" : "Admin Navigation (Link Cloaker)"}
                      </button>
                    </div>
                        ...
                        {isBlueprnt ? (
                          <>
                            <div className="p-3 bg-muted/30 rounded-xl border border-border text-xs space-y-1">
                              <div className="font-bold text-foreground">📷 Wedding &amp; High-Volume Event Photography</div>
                              <p className="text-muted-foreground">Ingest thousands of RAW+JPG pairs across multiple cards simultaneously and map student/client CSV rosters.</p>
                            </div>
                            <div className="p-3 bg-muted/30 rounded-xl border border-border text-xs space-y-1">
                              <div className="font-bold text-foreground">🎬 Cinema &amp; Commercial On-Set DIT Workstage</div>
                              <p className="text-muted-foreground">Offload cinema cards with BLAKE3 validation, mirror to secondary backup SSDs, and generate Apple ProRes/DNxHR proxies.</p>
                            </div>
                            <div className="p-3 bg-muted/30 rounded-xl border border-border text-xs space-y-1">
                              <div className="font-bold text-foreground">🛡️ Metadata Privacy Sanitation</div>
                              <p className="text-muted-foreground">Strip camera body serials, lens serials, binary MakerNotes, and GPS location coordinates before client delivery.</p>
                            </div>
                          </>
                        ) : (
                          <>
                          ...
                          <>
                            <div className="flex items-center justify-between border-b border-border pb-2">
                              <span className="font-bold text-foreground flex items-center gap-1.5">
                                <Laptop className="h-4 w-4 text-sky-500" /> Multi-Platform Desktop Builds
                              </span>
                              <span className="text-[11px] text-muted-foreground">100% Offline • Zero-Cloud</span>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 font-mono text-[11px]">
                              <span className="bg-background p-2 rounded border text-center"> macOS (Apple Silicon &amp; Universal)</span>
                              <span className="bg-background p-2 rounded border text-center">⊞ Windows 10/11 (x64)</span>
                              <span className="bg-background p-2 rounded border text-center">🐧 Linux (AppImage &amp; Deb)</span>
                            </div>
                          </>
                        ) : (
                          <>
                  ...
                  <div className="mb-8">
                    ...
                    <div className="flex flex-wrap gap-2 text-xs font-medium text-muted-foreground">
                      <span className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-bold px-3 py-1 rounded-md border border-emerald-500/30">
                        {isBlueprnt ? "100% Offline • Local-First (v1.1.0)" : "100% Feature Complete (v1.0.13)"}
                      </span>
                      <span className="bg-muted px-3 py-1 rounded-md border">
                        {isBlueprnt ? "Zero-Cloud Desktop Workstage" : "Submitted for WordPress.org Review"}
                      </span>
                      <span className="bg-muted px-3 py-1 rounded-md border">
